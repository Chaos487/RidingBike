...
public class BikeDamageSystem : MonoBehaviour
{
    ...
    /// <summary>当前血量,只读。供 Node 系统预判"选了这个 Choice 会不会把自己扣死"用
    ...
    /// 让当前血量凭空"多"出一截或者超过新的满血值)，正值当场回一部分血,负值当场扣一部分血。
    /// 如果扣到 0 或以下,跟真摔车归零一样直接判定为最终摔车——不然玩家会卡在"血量 0/1 但没有
    /// 判死"这种既没死也不能继续正常玩的状态。</summary>
