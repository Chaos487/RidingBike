...
**掉进断层**(`GapFallHandler.cs` + `GapFallSettings.cs`,3.12 节)
玩家没跳过断层、掉得比记录的地面高度深过阈值:镜头脱离跟随定在原地、扣一次血(跟
正常摔车共用同一条 HP)、车身交给物理自己继续往下掉(反正已经看不见),UI 弹出"按
Space 继续";按 Space 后车身传送回断层终点前方,镜头自动追回来,正常游戏继续。
